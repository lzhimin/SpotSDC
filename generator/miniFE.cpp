...
void waxpby(FTDouble a, FTDouble* x, FTDouble b, FTDouble* y, FTDouble* r) {
  int i;
  for (i = 0; i < N; ++i) {
    r[i] = x[i] * a + b * y[i]; //corruptDouble(VAR_INFO_ONLY(r),&r[i]);
  }
}
...
void readA(FTDouble** A, FILE *fp) {
  ...
  for (i = 0; i < N; i++) {
    for (j = 0; j < N; j++) {
      fscanf(fp, "%lf", &(A[i][j].value)); //corruptDouble(VAR_INFO_ONLY(A),&A[i][j]);
      //printf("%lf \n", A[i][j].value);
    }
  }
}
...
void readB(FTDouble* b, FILE *fp) {
  int i;
  for (i = 0; i < N; i++) {
    fscanf(fp, "%lf", &(b[i].value)); //corruptDouble(VAR_INFO_ONLY(b),&b[i]);
  }
}
