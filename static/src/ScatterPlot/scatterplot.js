class ScatterPlot extends BasicView {
    ...
    draw() {
        this.circle_r = 3;
        this.x_axis = d3.scaleLinear().domain([0, d3.max(this.data, (d) => {
            return d.x;
        })]).range([this.margin.left, this.width - this.margin.right]);
        this.y_axis = d3.scaleLinear().domain([0, d3.max(this.data, (d) => {
            return d.y;
        ...
        let x_histogram_y = d3.scaleLinear()
            ...
            .domain([0, d3.max(x_histogram_bin, (d) => {
                return d.length;
            })]);
        ...
        this.chart.append('g').selectAll('rect').data(x_histogram_bin).enter()
            ...
            .attr("x", 1)
            .attr("transform", (d) => {
                return "translate(" + this.x_axis(d.x0) + "," + (this.height - this.margin.bottom + 20) + ")";
            })
            .attr("width", (d) => {
                return this.x_axis(d.x1) - this.x_axis(d.x0) - 1;
            })
            .attr("height", (d) => {
                return this.margin.bottom - 50 - x_histogram_y(d.length);
            })
            .style("fill", "gray");
        ...
        let y_histogram_x = d3.scaleLinear()
            ...
            .domain([0, d3.max(y_histogram_bin, (d) => {
                return d.length;
            })]);
