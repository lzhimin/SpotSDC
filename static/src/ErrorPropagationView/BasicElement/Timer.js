class Timer {
    ...
    draw() {
        ...
        this.axis_y = d3.scaleLinear().range([this.y, this.y - this.height]).domain([Math.min(0, d3.min(this.absoluteError, (d) => {
            return d[1];
        })), Math.max(1, d3.max(this.absoluteError, (d) => {
            return d[1];
        }))]);
        ...
        this.linefunc = d3.line().x((d, i) => {
            return this.axis_x(i);
        }).y((d, i) => {
            return this.axis_y(d[1]);
    ...
    redraw() {
        ...
        this.axis_y = d3.scaleLinear().range([this.y, this.y - this.height]).domain([Math.min(0, d3.min(this.absoluteError, (d) => {
            ...
            return d[1];
        }))]); //relative error the value scale from 0~1
    ...
    getFirstErrorIndex() {
        ...
        let index = -1;
        for (let i = 0; i < this.absoluteError.length; i++) {
            if (this.absoluteError[i][1] > 1e-8) {
                index = i;
                break;
            }
        }
